...
public sealed class EjecucionOrchestratorTests
{
    ...
    [Fact]
    public async Task ReintentarPasoAsync_OnAFailedStep_CreatesANewAttemptWithoutTouchingTheFailedOne()
    {
        using var db = TestDbContextFactory.Create();
        var orchestrator = new EjecucionOrchestrator(db, BuildExecutorProvider());
        ...
        await orchestrator.ReintentarPasoAsync(pasoFallido.Id, default);
